...
public struct MonthlyCalendarView: View, MonthlyCalendarManagerDirectAccess {
    ...
    private var monthsList: some View {
        ...
        ScrollViewReader { reader in
            ScrollView(.vertical) {
                LazyVStack {
                    ForEach(Array(calendarManager.months.enumerated()), id: \.offset) { index, month in
                        print("Month ID: \(month.timeIntervalSince1970)")
                ...
                //                    MonthView(calendarManager: calendarManager, month: month)
                //                }
            }
            //            .task {
            //                try? await Task.sleep(nanoseconds: 7_500_000_000)
            //                reader.scrollTo(calendarManager.currentMonth.timeIntervalSince1970)
            //            }
            .onAppear() {
                if firstTime {
